...
describe("MockedCall()", () => {
    ...
    describe("didRun()", () => {
        it("Fails as no returns()", () => {
            const mockedCall = new MockedCall("m", [3])
                .returns(() => 5)
                .times(0);
        ...
        it("Fails as actualTimes === expectedTimes", () => {
            const mockedCall = new MockedCall("m", [4])
                .times(0)
                .returns(f);
            assertThat(mockedCall.didRun([1]).isSome).is(false);
        });
